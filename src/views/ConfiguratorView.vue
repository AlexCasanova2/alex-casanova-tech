...
<template>
  <main class="configurator page-wrapper container fade-in">
    <header class="config-head"><div><span class="eyebrow">{{ copy.eyebrow }}</span><h1>{{ copy.title }}</h1><p>{{ copy.intro }}</p></div><div class="progress" :aria-label="`Paso ${step + 1} de 5`"><span v-for="index in 5" :key="index" :class="{ active:index - 1 <= step }"></span><small>0{{ step + 1 }} / 05</small></div></header>
    <div class="config-grid">
      <section class="question-panel">
        <div v-if="!sent" class="question-copy"><span class="step-number">0{{ step + 1 }}</span><h2>{{ copy.steps[step].title }}</h2><p>{{ copy.steps[step].help }}</p></div>
        ...
        <form v-else-if="!sent" class="lead-form" @submit.prevent="submit">
          <div class="field"><label>{{ copy.labels.name }}</label><input v-model="contact.name" :aria-label="copy.labels.name" required minlength="2" autocomplete="name"></div><div class="field"><label>{{ copy.labels.email }}</label><input v-model="contact.email" :aria-label="copy.labels.email" required type="email" autocomplete="email"></div><div class="field"><label>{{ copy.labels.company }}</label><input v-model="contact.company" :aria-label="copy.labels.company" autocomplete="organization"></div><div class="field"><label>{{ copy.labels.website }}</label><input v-model="contact.website" :aria-label="copy.labels.website" inputmode="url" placeholder="https://"></div><div class="field wide"><label>{{ copy.labels.message }}</label><textarea v-model="contact.message" :aria-label="copy.labels.message" rows="3"></textarea></div><div class="trap" aria-hidden="true"><label>Company name<input v-model="contact.companyName" tabindex="-1" autocomplete="off"></label></div><label class="privacy wide"><input v-model="contact.privacyAccepted" type="checkbox" :aria-label="copy.labels.privacy" required> <span>{{ copy.labels.privacy }} <router-link :to="lang === 'ca' ? '/ca/privacitat' : '/es/privacidad'" target="_blank">{{ lang === 'ca' ? 'Més informació' : 'Más información' }}</router-link>.</span></label><p v-if="error" class="form-error wide" role="alert">{{ error }}</p><button class="btn btn-primary wide" :disabled="isSubmitting">{{ isSubmitting ? copy.sending : copy.send }} →</button>
        </form>
